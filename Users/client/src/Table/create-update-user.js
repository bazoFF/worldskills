...
function CreateUpdateUser({hideNewUser, createUpdateUser, users} ) {
    let [nameVal, setNameVal] = useState('');
    let [secondNameVal, setSecondNameVal] = useState('');
    let [emailNameVal, setEmailVal] = useState('');
    ...
    const handleAddNewUser = () => {
        if (nameVal.length > 0 || secondNameVal > 0 || secondNameVal > 0) {
            createUpdateUser(nameVal, secondNameVal, emailNameVal);
        } else {
    ...
    if(users.id) {
        return <tr className={'new-user'}>
            ...
            <td><input type="text" value={ emailNameVal } onChange={ updateEmailVal } placeholder={"Введите email"}/></td>
